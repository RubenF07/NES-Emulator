...
pub fn render(ppu: &NesPPU, frame: &mut Frame){
    // Background
    let scroll_x = (ppu.scroll.x_scroll) as usize;
    let scroll_y = (ppu.scroll.y_scroll) as usize;
...
fn bg_palette(ppu: &NesPPU, attribute_table: &[u8], tile_column: usize, tile_row: usize) -> [u8;4]{
    ...
    let attr_byte = attribute_table[attr_table_idx]; // hardcoded first nametable
...
fn render_name_table(ppu: &NesPPU, frame: &mut Frame, name_table: &[u8], view_port: Rect, shift_x: isize, shift_y: isize){
    let bank = ppu.ctrl.bkgr_ptrn_addr();
